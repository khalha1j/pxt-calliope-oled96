...
namespace oled96 {
    ...
    function clearDisplayLine(line: number) {
        //cmd(DISPLAY_OFF);   //display off
    ...
    /**
    * Writes a (عربي) HAK 2.38 string to the display at the current cursor position.
    */
    //% blockId=oled96_write_string_Arabic
    //% block="write Arabic (عربي) %s|to display at line %line"
    //% %line.min =0 %line.max=7
    export function writeArabicString(s: string, line: number) {
        let pos = 0;
        let useless = 0;
        let posNext = 0;
        let posPrev = 0;
    ...
    /**
    * Writes a (English) HAK 2.38 string to the display at the current cursor position.
    */
    //% blockId=oled96_write_string_english
    //% block="write (English) %s|to display at line %line"
    //% %line.min =0 %line.max=7
    export function writeEnglishString(s: string, line: number) {
...
const basicFont_arabic_starting: string[] = [
    ...
    "\x00\x74\xFE\xC7\xC5\xC5\xC0\xF8\x7C\x0C\x1C\x38\x00\x00\x00\x00", // "&"  6   ئ
    ...
    "\x30\x30\x30\x33\x30\x36\x37\x3D\x1F\x00\x00\x00\x00\x00\x00\x00", // "A"  33  ف
    "\x30\x33\x30\x33\x30\x36\x37\x3D\x3F\x3F\x00\x00\x00\x00\x00\x00", // "B"  34  ق
...
const basicFont_arabic_separate: string[] = [
    ...
    "\x00\x06\x73\xFB\xFB\xCF\xCF\xC7\xD7\xD6\xC4\x00\x00\x00\x00\x00", // ","  12  ج
    ...
    "\x00\x1E\xBF\xB0\x30\xB0\xB6\x3F\x39\x33\x03\x00\x00\x00\x00\x00", // "J"  42  ي
    "\x00\x7F\x08\x14\x22\x41\x00\x00\x00\x7F\x08\x14\x22\x41\x00\x00", // "K"  43  تنوين فتح ً
    "\x00\x7F\x40\x40\x40\x40\x00\x00\x00\x7F\x40\x40\x40\x40\x00\x00", // "L"  44  تنوين ضم  ٌ
    "\x00\x7F\x02\x0C\x02\x7F\x00\x00\x00\x7F\x02\x0C\x02\x7F\x00\x00", // "M"  45
    "\x00\x7F\x04\x08\x10\x7F\x00\x00\x00\x7F\x04\x08\x10\x7F\x00\x00", // "N"  46 َفتحة
...
const basicFont_arabic_ending: string[] = [
    ...
    "\x00\x00\x00\x00\x00\x19\x1D\x14\x1D\x3D\x30\x30\x00\x00\x00\x00", // ")"  9   ة
